...
if (!['arm64', 'x64'].includes(architecture)) {
  throw new Error(`Unsupported agent sender architecture: ${architecture}`);
}

const targetArchitecture = architecture === 'x64' ? 'x86_64' : 'arm64';
const outputDirectory = path.join(root, 'native', 'build', architecture);
const outputPath = path.join(outputDirectory, 'blobfish-agent-event-sender');
const sourcePath = path.join(root, 'native', 'AgentEventSender.swift');
