...
async function run(operation, successMessage) {
  try {
    ...
    showMessage(error.message || '刚才没有设置成功。');
    return null;
  }
}
...
function renderTimer() {
  ...
  byId('timer-state').textContent = timer
    ? (timer.state === 'running' ? '正在计时' : '已暂停')
    : '还没有开始';
  if (!timer) return;
  byId('timer-label').textContent = timer.label || '计时中';
  byId('timer-countdown').textContent = formatDuration(activeTimerRemaining(timer));
  byId('toggle-timer').textContent = timer.state === 'running' ? '暂停' : '继续';
}

function alarmRepeatLabel(alarm) {
  if (alarm.mode === 'once') return alarm.date;
  if (alarm.mode === 'daily') return '每天';
  if (alarm.mode === 'workdays') return '工作日';
  const names = ['日', '一', '二', '三', '四', '五', '六'];
  return `周${alarm.weekdays.map((day) => names[day]).join('、')}`;
}
...
function renderAlarmList() {
  ...
  byId('alarm-count').textContent = `${alarms.length} 个`;
  ...
  for (const alarm of alarms) {
    ...
    enabled.setAttribute('aria-label', `${alarm.label || alarm.time}启用状态`);
    ...
    label.textContent = `${alarm.label || '闹钟'} · ${alarmRepeatLabel(alarm)}`;
    ...
    remove.addEventListener('click', () => {
      if (!window.confirm(`删除“${alarm.label || nextAlarmTime(alarm)}”？`)) return;
      void run(() => window.clockAPI.deleteAlarm(alarm.id), '闹钟已删除。');
    });
...
function renderAlerts() {
  ...
  if (!alert) return;
  byId('ringing-kind').textContent = alert.sourceType === 'alarm' ? '闹钟到了' : '计时结束';
  byId('ringing-title').textContent = alert.label || '时间到了';
...
function resetAlarmForm() {
  ...
  byId('save-alarm').textContent = '设置闹钟';
...
function beginAlarmEdit(alarm) {
  ...
  byId('save-alarm').textContent = '保存修改';
...
function alarmFormValue() {
  ...
  if (mode === 'weekly' && weekdays.length === 0) throw new Error('请至少选择一个星期');
  return {
...
byId('toggle-timer').addEventListener('click', () => {
  ...
  void run(
    ...
    timer.state === 'running' ? '计时已暂停。' : '继续计时。',
  );
});
byId('extend-timer').addEventListener('click', () => {
  void run(() => window.clockAPI.extendTimer(5), '增加了 5 分钟。');
});
byId('cancel-timer').addEventListener('click', () => {
  void run(() => window.clockAPI.cancelTimer(), '计时已取消。');
});
...
byId('snooze-alert').addEventListener('click', () => {
  ...
  if (id) void run(() => window.clockAPI.snoozeAlert(id, 5), '五分钟后再提醒。');
});
byId('dismiss-alert').addEventListener('click', () => {
  const id = byId('dismiss-alert').dataset.alertId;
  if (id) void run(() => window.clockAPI.dismissAlert(id), '提醒已关闭。');
});
